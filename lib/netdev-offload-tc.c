...
{
    dst->type = src->type;
    dst->action = nullable_xmemdup(src->action,
                                   src->action ? src->action->nla_len : 0);
    ...
    dst->userdata = nullable_xmemdup(src->userdata,
                                     src->userdata ? src->userdata->nla_len
                                                   : 0);
...
sgid_alloc(const struct offload_sample *sample)
{
    ...
    if (id_pool_alloc_id(sample_group_ids, &node->id)) {
        ...
        VLOG_ERR_RL(&error_rl, "Can't find a free sample group ID");
        sgid_node_free(node);
        node = NULL;
    }
...
sgid_free(uint32_t id)
{
    ...
    if (node) {
        ...
        VLOG_ERR_RL(&error_rl,
                    "Freeing nonexistent sample group ID: %"PRIu32, id);
    }
}
 ...
 * @sample_group_id: Mapping id for sample actions.  Currently, only supports
                     a single sFlow action.  Using an array to support multiple
        ...
        case TC_ACT_SAMPLE: {
            ...
            if (!node) {
                VLOG_WARN_RL(&warn_rl,
                          "Can't find sample group ID data for ID: %"PRIu32,
        ...
        if (nl_attr_type(nla) == OVS_ACTION_ATTR_OUTPUT) {
            ...
            *recirc_act = true;
            ...
            if (get_flower_sgid_count(flower) >= MAX_TC_SAMPLES_PER_FLOW) {
                VLOG_DBG_RL(&rl,
                            "Only %u TC_SAMPLE actions per flow are supported",
                            MAX_TC_SAMPLES_PER_FLOW);
                return EOPNOTSUPP;
            }
