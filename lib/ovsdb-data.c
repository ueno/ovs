...
ovsdb_datum_clone(struct ovsdb_datum *new, const struct ovsdb_datum *old,
                  const struct ovsdb_type *type OVS_UNUSED)
{
    struct ovsdb_datum *old_w = CONST_CAST(struct ovsdb_datum *, old);
    unsigned int n = old->n;

    if (!old->refcnt) {
        old_w->refcnt = xmalloc(sizeof *old_w->refcnt);
        *old_w->refcnt = 1;
    }
    new->n = n;
    new->keys = old_w->keys;
    new->values = old_w->values;
    new->refcnt = old_w->refcnt;
    (*new->refcnt)++;
}
...
{
    if (!datum->refcnt || --(*datum->refcnt) == 0 ) {
