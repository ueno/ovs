...
raft_precheck_prereq(const struct raft *raft, const struct uuid *prereq)
{
    ...
    if (raft->role != RAFT_LEADER && hmap_count(&raft->commands)) {
        ...
        HMAP_FOR_EACH (cmd, hmap_node, &raft->commands) {
            /* It is possible that we still have incomplete commands from
             * the time this server was a leader, skip those.  Also skip
             * commands that do not carry any data, as they can't change
             * prerequisites.  This isn't just an optimization, because
             * the database will not re-run triggers unless data changes
             * or one of the data-carrying triggers completes. */
            if (!cmd->index && !uuid_is_zero(&cmd->eid)) {
                VLOG_INFO("follower still an incomplete command "UUID_FMT,
                          UUID_ARGS(&cmd->eid));
                return false;
            }
        }
    ...
    VLOG_INFO("prerequisites "UUID_FMT" match!", UUID_ARGS(prereq));
    return true;
}
