             ...
             bool by_key, const struct ovsdb_column *column)
{
    ...
    if (column->type.value.type != OVSDB_TYPE_VOID) {
        /* Cloning the value too in order to avoid accidental deletion of the
         * replaced key-value in case reference was added by the old value. */
    ...
    SHASH_FOR_EACH (node, &table->schema->columns) {
        ...
        if (ovsdb_base_type_is_weak_ref(&column->type.key)) {
            for (i = 0; i < added.n; i++) {
                ...
                row = ovsdb_table_get_row(column->type.key.uuid.refTable,
                                          &added.keys[i].uuid);
                if (row) {
                    add_weak_ref(txn_row, row, &txn_row->added_refs,
                                 &added.keys[i],
                                 added.values ? &added.values[i] : NULL,
                                 true, column);
                } else {
                    if (uuid_is_zero(&added.keys[i].uuid)) {
                        zero = true;
                    }
                    ovsdb_datum_add_unsafe(&deleted_refs,
                                           &added.keys[i], &added.values[i],
                                           &column->type, NULL);
                }
            }
        }
        ...
        if (ovsdb_base_type_is_weak_ref(&column->type.value)) {
            for (i = 0; i < added.n; i++) {
                ...
                if (row) {
                    ...
                    if (uuid_is_zero(&added.values[i].uuid)) {
                        zero = true;
                    }
                    ovsdb_datum_add_unsafe(&deleted_refs,
                                           &added.keys[i], &added.values[i],
        ...
        if (ovsdb_base_type_is_weak_ref(&column->type.key)) {
            for (i = 0; i < removed.n; i++) {
                ...
                row = ovsdb_table_get_row(column->type.key.uuid.refTable,
                                          &removed.keys[i].uuid);
                if (!row) {
                    /* Row is already deleted or never existed. */
                    continue;
                }
                add_weak_ref(txn_row, row, &txn_row->deleted_refs,
                             &removed.keys[i],
                             removed.values ? &removed.values[i] : NULL,
                             true, column);
            }
        }
        ...
        if (ovsdb_base_type_is_weak_ref(&column->type.value)) {
            for (i = 0; i < removed.n; i++) {
                ...
                row = ovsdb_table_get_row(column->type.value.uuid.refTable,
                                          &removed.values[i].uuid);
                /* This one should exist. */
                ovs_assert(row);
                add_weak_ref(txn_row, row, &txn_row->deleted_refs,
                             &removed.keys[i], &removed.values[i],
                             false, column);
            }
        }
        ovsdb_datum_destroy(&removed, &column->type);
